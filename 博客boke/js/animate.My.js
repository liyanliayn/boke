        ...
        for(var key in json){
            ...
            if(key == "opacity"){
                ...
                //leader = parseInt(getStyle(elem, key) * 100) || 100;
                 leader =  Math.round(getStyle(elem,key) * 100) || 100;
            }else {
                leader = parseInt(getStyle(elem, key)) || 0;//当前的值
            }
            ...
            //console.log(json[key]+"===="+parseInt(leader) +parseInt(step));
            if(json[key] != leader ){
                flag = false;
            }
        }
...
//        var speed=target>obj.offsetLeft?Math.ceil(temp):Math.floor(temp);
